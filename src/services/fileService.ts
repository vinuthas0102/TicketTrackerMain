    ...
    try {
      ...
      if (insertError) {
        ...
        if (insertError.code === '42501') {
          throw new Error(
            `Failed to save document metadata: Permission denied. ` +
            `This may be due to database security policies. Error: ${insertError.message}`
          );
        }
      ...
      if (fileReferenceId) {
        try {
          ...
          console.error('Failed to link document to file reference:', linkError);
        }
      }
  ...
  static async getStepProgressHistory(stepId: string): Promise<ProgressHistoryEntry[]> {
    ...
    try {
      ...
      (auditLogs.data || []).forEach(log => {
        ...
        const userComment = log.metadata?.comment || log.metadata?.remarks;
        const baseEntry = {
          id: log.id,
          ...
          comment: userComment || log.description,
          auditLogId: log.id,
        ...
        if (log.action === 'PROGRESS_DOCUMENTS_UPLOADED' || (log.action_category === 'document_action' && log.action.includes('PROGRESS'))) {
          ...
          const progressVal = log.metadata?.progress !== undefined
            ? log.metadata.progress
            : (log.new_data && !isNaN(Number(log.new_data)) ? Number(log.new_data) : undefined);
          if (progressVal !== undefined || userComment || log.description) {
            const docs = progressDocsMap.get(log.id) || [];
            entries.push({
              ...baseEntry,
              type: 'progress_update',
              progress: progressVal,
              oldProgress: log.metadata?.old_progress !== undefined
                ? log.metadata.old_progress
                : (log.old_data && !isNaN(Number(log.old_data)) ? Number(log.old_data) : undefined),
