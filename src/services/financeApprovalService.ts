...
export class FinanceApprovalService {
  ...
  static async approveFinanceRequest(decision: FinanceApprovalDecision, decidedBy: string): Promise<void> {
    try {
      ...
      if (approvalUpdateError) {
        console.error('Finance approval update error:', approvalUpdateError);
        throw new Error(`Failed to update finance approval status: ${approvalUpdateError.message || 'Unknown error'}`);
      }
      ...
      if (ticketUpdateError) {
        console.error('Ticket update error during approval:', ticketUpdateError);
        throw new Error(`Failed to update ticket status: ${ticketUpdateError.message || 'Unknown error'}`);
      }
  ...
  static async rejectFinanceRequest(decision: FinanceApprovalDecision, decidedBy: string): Promise<void> {
    try {
      ...
      if (approvalUpdateError) {
        console.error('Finance approval rejection update error:', approvalUpdateError);
        throw new Error(`Failed to update finance approval status: ${approvalUpdateError.message || 'Unknown error'}`);
      }
      ...
      if (ticketUpdateError) {
        console.error('Ticket update error during rejection:', ticketUpdateError);
        throw new Error(`Failed to update ticket status: ${ticketUpdateError.message || 'Unknown error'}`);
      }
