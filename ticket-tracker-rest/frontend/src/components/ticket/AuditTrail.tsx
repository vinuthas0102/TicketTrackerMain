...
const AuditTrail: React.FC<AuditTrailProps> = ({ ticket, viewingDocument, onCloseDocument, onViewProgressDocument, viewingChat, onCloseChat }) => {
  ...
  React.useEffect(() => {
    if (viewingDocument?.document && viewingDocument.document.id) {
      setLoadingUrl(true);

      FileService.getFileUrl(viewingDocument.document.id)
        .then((url) => {
          setDocumentUrl(url);
        })
        .catch((error) => {
      ...
      <div className="flex flex-col" style={{ minHeight: '400px', height: '100%' }}>
        ...
        <div className="flex-1 overflow-hidden">
          <WorkflowStepComments
            ...
            parentStepAssignedTo={parentStepAssignedTo}
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-1.5 text-xs text-gray-600 mt-0.5">
              ...
              <span className="flex items-center space-x-1">
                <span className="text-2xl">{FileService.getFileIcon(viewingDocument.document.type || 'application/octet-stream')}</span>
