...
const WorkflowDocumentUpload: React.FC<WorkflowDocumentUploadProps> = ({
  ...
  const handleFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        const isCompletionCert = isDORole ? isMandatory : false;
  ...
  const handleDownload = async (doc: DocumentMetadata) => {
    try {
      await FileService.downloadFile(doc.id, doc.name);
    } catch (error) {
              ...
              >
                <div className="flex items-center space-x-3 flex-1 min-w-0">
                  <span className="text-2xl flex-shrink-0">
                    {FileService.getFileIcon(doc.type || 'application/octet-stream')}
                  </span>
