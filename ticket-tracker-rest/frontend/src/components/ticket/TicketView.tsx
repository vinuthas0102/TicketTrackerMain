...
import { ArrowLeft, Calendar, User, AlertTriangle, Clock, CheckCircle, XCircle, FileText, Users, CreditCard as Edit, Trash2, Plus, Paperclip, Download, Trash, Upload, IndianRupee } from 'lucide-react';
...
const TicketView: React.FC<TicketViewProps> = ({ ticket, onClose, onEdit, onDelete }) => {
  ...
  const handleDownloadAttachment = async (attachment: DocumentMetadata) => {
    try {
      await FileService.downloadFile(attachment.id, attachment.name);
    } catch (error) {
                  ...
                  <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                    ...
                    {ticket.requestType && (
                      <div>
                        ...
                        <span className="text-sm text-gray-900">{ticket.requestType}</span>
                      </div>
                    )}
                  </div>
                          ...
                          >
                            <div className="flex items-center space-x-2 flex-1 min-w-0">
                              <div className="text-xl flex-shrink-0">
                                {FileService.getFileIcon(attachment.type || 'application/octet-stream')}
                              </div>
