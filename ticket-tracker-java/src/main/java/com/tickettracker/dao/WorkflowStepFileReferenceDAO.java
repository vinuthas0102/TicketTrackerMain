...
public class WorkflowStepFileReferenceDAO extends BaseDAO {
    ...
    public List<WorkflowStepFileReference> findByStepId(byte[] stepId) throws SQLException {
        String sql = "SELECT * FROM workflow_step_file_references WHERE step_id = ? " +
                "ORDER BY is_mandatory DESC, reference_name";
    ...
    public List<Map<String, Object>> findFileReferencesWithDocumentDetails(byte[] stepId) throws SQLException {
        ...
        try {
            ...
            logger.info("Fetched {} file references with document details for step {}", results.size(), bytesToHex(stepId));
            return results;
