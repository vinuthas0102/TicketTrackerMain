...
public class AuditLogDAO extends BaseDAO {
    ...
    public AuditLog create(AuditLog auditLog) throws SQLException {
        ...
        try {
            ...
            if (!originalAutoCommit) {
                ...
                logger.debug("Auto-commit was disabled, enabled for audit log creation");
            }
            ...
            logger.debug("Creating audit log - ID: {}, TicketID: {}, StepID: {}, PerformedBy: {}, Action: {}, Category: {}",
                    bytesToHex(id), bytesToHex(auditLog.getTicketId()), bytesToHex(auditLog.getStepId()),
                    bytesToHex(auditLog.getPerformedBy()), auditLog.getAction(), auditLog.getActionCategory());
            ...
            String verifyQuery = "SELECT COUNT(*) FROM audit_logs WHERE id = ?";
            PreparedStatement verifyStmt = conn.prepareStatement(verifyQuery);
            verifyStmt.setBytes(1, id);
            ResultSet verifyRs = verifyStmt.executeQuery();
            if (verifyRs.next()) {
                int count = verifyRs.getInt(1);
                logger.debug("Verification query: Found {} records with ID {}", count, bytesToHex(id));
            }
            verifyRs.close();
            verifyStmt.close();
            ...
            logger.error("Failed to create audit log - StepID: {}, Action: {}, Error: {}",
                    bytesToHex(auditLog.getStepId()), auditLog.getAction(), e.getMessage(), e);
            throw e;
        } finally {
    ...
    public List<AuditLog> findByStepId(byte[] stepId) throws SQLException {
        ...
        logger.debug("Querying audit logs for StepID: {}", bytesToHex(stepId));
        ...
        try {
            ...
            logger.debug("Found {} audit logs for StepID: {}", auditLogs.size(), bytesToHex(stepId));
