...
public class TicketDAO extends BaseDAO {
    ...
    public Ticket create(Ticket ticket) throws SQLException {
        String sql = "INSERT INTO tickets (id, ticket_number, module_id, title, description, status, " +
                ...
                "completion_documents_required, requires_finance_approval, start_date) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public Ticket findById(byte[] id, boolean loadRelatedData) throws SQLException {
    ...
    public List<Ticket> findAll(boolean loadRelatedData) throws SQLException {
    ...
    public List<Ticket> findByModuleId(byte[] moduleId) throws SQLException {
        return findByModuleId(moduleId, true);
    }

    public List<Ticket> findByModuleId(byte[] moduleId, boolean loadRelatedData) throws SQLException {
    ...
    public List<Ticket> findByStatus(String status, boolean loadRelatedData) throws SQLException {
    ...
    public List<Ticket> findByAssignedTo(byte[] userId) throws SQLException {
        return findByAssignedTo(userId, true);
    }

    public List<Ticket> findByAssignedTo(byte[] userId, boolean loadRelatedData) throws SQLException {
    ...
    public Ticket update(Ticket ticket) throws SQLException {
        String sql = "UPDATE tickets SET module_id = ?, title = ?, description = ?, status = ?, " +
                ...
                "requires_finance_approval = ?, start_date = ?, updated_at = CURRENT_TIMESTAMP " +
                "WHERE id = ?";
    ...
    public List<Ticket> searchTickets(String searchTerm) throws SQLException {
        return searchTickets(searchTerm, true);
    }

    public List<Ticket> searchTickets(String searchTerm, boolean loadRelatedData) throws SQLException {
    ...
    public List<Ticket> findAccessibleTickets(byte[] userId, String userRole) throws SQLException {
        return findAccessibleTickets(userId, userRole, true);
    }

    public List<Ticket> findAccessibleTickets(byte[] userId, String userRole, boolean loadRelatedData) throws SQLException {
        String sql;
        ...
        if ("Admin".equalsIgnoreCase(userRole) || "Finance".equalsIgnoreCase(userRole)) {
            ...
            sql = "SELECT * FROM tickets WHERE created_by = ? OR assigned_to = ? " +
                    "ORDER BY created_at DESC";
    ...
    private void loadTicketRelatedData(Ticket ticket) {
        ...
        try {
            ...
            logger.debug("Loaded {} workflow steps for ticket: {}", workflowSteps.size(), bytesToHex(ticket.getId()));
        } catch (SQLException e) {
            logger.warn("Failed to load workflow steps for ticket {}: {}", bytesToHex(ticket.getId()), e.getMessage());
            ticket.setWorkflow(new ArrayList<>());
        }

        try {
            List<FileAttachment> attachments = fileAttachmentDAO.findByTicketId(ticket.getId());
            ticket.setAttachments(attachments);
            logger.debug("Loaded {} attachments for ticket: {}", attachments.size(), bytesToHex(ticket.getId()));
        } catch (SQLException e) {
            logger.warn("Failed to load attachments for ticket {}: {}", bytesToHex(ticket.getId()), e.getMessage());
        ...
        try {
            ...
            logger.debug("Loaded {} audit logs for ticket: {}", auditLogs.size(), bytesToHex(ticket.getId()));
        } catch (SQLException e) {
            logger.warn("Failed to load audit logs for ticket {}: {}", bytesToHex(ticket.getId()), e.getMessage());
