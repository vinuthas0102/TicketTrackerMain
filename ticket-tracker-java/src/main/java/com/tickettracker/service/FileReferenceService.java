        ...
        try {
            ...
            if (updates.containsKey("uploadedAt") && updates.get("uploadedAt") != null) {
                ...
                if (uploadedAtObj instanceof java.util.Date) {
                    ...
                    String uploadedAtStr = (String) uploadedAtObj;
                    try {
                        java.time.Instant instant = java.time.Instant.parse(uploadedAtStr);
                        reference.setUploadedAt(new java.sql.Timestamp(instant.toEpochMilli()));
                    } catch (java.time.format.DateTimeParseException e1) {
                        try {
                            reference.setUploadedAt(java.sql.Timestamp.valueOf(uploadedAtStr));
