...
public class WorkflowStepProgressDocumentService {
    ...
    public ProgressDocument uploadProgressDocument(ProgressDocument document) throws TicketTrackerException {
        ...
        if (document.getFilePath() == null || document.getFilePath().trim().isEmpty()) {
            document.setFilePath("blob_" + System.currentTimeMillis());
        }
    ...
    public List<ProgressDocument> getProgressDocumentsByStepId(byte[] stepId) throws TicketTrackerException {
        String stepIdHex = bytesToHex(stepId);
        logger.info("Fetching progress documents for step: {}", stepIdHex);
        try {
            List<ProgressDocument> documents = progressDocumentDAO.findByStepId(stepId);
            logger.debug("Found {} progress documents for StepID: {}", documents.size(), stepIdHex);
            return documents;
        } catch (SQLException e) {
            logger.error("Error fetching progress documents by step ID: {}", stepIdHex, e);
