...
public class FinanceApprovalService {
    ...
    public FinanceApproval approveFinanceApproval(byte[] approvalId, byte[] currentUserId)
            throws TicketTrackerException {
        try {
            FinanceApproval approval = getFinanceApprovalById(approvalId);

            User currentUser = userDAO.findById(currentUserId);
            if (currentUser == null || !"finance".equalsIgnoreCase(currentUser.getRole())) {
            ...
            FinanceApproval updatedApproval = financeApprovalDAO.approve(approvalId);
